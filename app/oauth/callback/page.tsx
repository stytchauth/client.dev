  ...
  useEffect(() => {
    const processCallback = async () => {
      try {
        ...
        if (!storedStateStr) {
          const errorMsg =
            "No OAuth flow state found. Please restart the flow.";
          setStatus("error");
          setMessage(errorMsg);
        ...
        >
          <CardContent className="p-6">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p
                  className={`font-semibold ${status === "loading" ? "text-gray-900" : status === "success" ? "text-green-900" : "text-red-900"}`}
                >
                  {status === "loading"
                    ? "Processing..."
                    : status === "success"
                ...
                <p
                  className={`text-sm mt-1 ${status === "loading" ? "text-gray-700" : status === "success" ? "text-green-800" : "text-red-800"}`}
                >
