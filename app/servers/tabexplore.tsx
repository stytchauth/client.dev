...
export function ExploreContent() {
  ...
  const [flowSteps, setFlowSteps] = useState<
    Array<{ text: string; status: "pending" | "loading" | "success" | "error" }>
  >([]);
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data && event.data.type === "oauth-callback") {
        ...
        setFlowSteps([
          ...
          {
            text: "Exchanging authorization code for tokens",
            status: "loading",
          },
    ...
    try {
      ...
      if (inputMode === "issuer") {
        ...
        if (!authServerMetadata.authorization_endpoint) {
          setError(
            "No authorization endpoint found in authorization server metadata",
          );
          setIsValidating(false);
          return;
        }
        ...
        try {
          ...
          if (
            !authUrl.protocol.startsWith("https") ||
            !tokenUrl.protocol.startsWith("https")
          ) {
        ...
        <div className="flex space-x-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-700">
              Test your authorization server's CIMD support. Provide your issuer
              URL for automatic endpoint discovery, or manually specify your
              authorization and token endpoints. We'll initiate an OAuth flow
              using client.dev's metadata document.
            </p>
          </div>
        </div>
          ...
          <CardContent className="space-y-4">
            ...
            {inputMode === "issuer" ? (
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  The base URL of your authorization server. We'll discover
                  endpoints from{" "}
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              <div className="text-sm text-gray-600 space-y-1 font-mono">
                <div>
                  ...
                  https://client.dev/oauth/metadata.json
                </div>
                <div>
                ...
                <div>
                  <strong>code_challenge_method:</strong> S256 (PKCE enabled)
                </div>
              </div>
            </div>
            ...
            {codeVerifier && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-2">
                ...
                <p className="text-xs text-green-700 mb-2">
                  Save this code_verifier - you'll need it to exchange the
                  authorization code for tokens at your token endpoint.
                </p>
            ...
            {flowSteps.length > 0 && (
              <div className="mt-4 space-y-4">
                  ...
                  <div className="space-y-3">
                    {flowSteps.map((step, index) => (
                      <div key={index} className="flex items-center space-x-3">
                        ...
                        <span
                          className={`text-sm ${step.status === "error" ? "text-red-700" : "text-gray-700"}`}
                        >
                          {step.text}
                        </span>
                      </div>
                ...
                {oauthResult && (
                  <div
                    className={`p-3 rounded-md ${oauthResult.status === "success" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <p
                          className={`font-semibold text-sm ${oauthResult.status === "success" ? "text-green-900" : "text-red-900"}`}
                        >
                          {oauthResult.status === "success"
                            ? "Success!"
                            : "Error"}
                        ...
                        <p
                          className={`text-xs mt-1 ${oauthResult.status === "success" ? "text-green-800" : "text-red-800"}`}
                        >
                          {oauthResult.message}
                        </p>
                      </div>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              {inputMode === "issuer" ? (
                <>
                  <p className="font-medium">
                    When you click "Start OAuth Flow", the tester will:
                  </p>
                  <ol className="list-decimal list-inside space-y-1 text-gray-600 ml-2">
                    <li>
                      Fetch{" "}
                      <code className="bg-gray-100 px-1 rounded">{`{issuer}/.well-known/oauth-authorization-server`}</code>
                    </li>
                    <li>
                      Extract the{" "}
                      <code className="bg-gray-100 px-1 rounded">
                    ...
                    <li>
                      Redirect to the authorization endpoint with PKCE
                      parameters
                    </li>
                  </ol>
                </>
                ...
                <>
                  ...
                  <ol className="list-decimal list-inside space-y-1 text-gray-600 ml-2">
                    <li>
                      Use the authorization and token endpoints you provided
                    </li>
                    <li>
                      Redirect to the authorization endpoint with PKCE
                      parameters
              ...
              <p className="font-medium mt-4">
                Your authorization server should then:
              </p>
              <ol
                className="list-decimal list-inside space-y-1 text-gray-600 ml-2"
                start={inputMode === "issuer" ? 4 : 3}
              >
                <li>
                  Receive the authorization request with{" "}
                  <code className="bg-gray-100 px-1 rounded">client_id</code> as
                  a URL
                </li>
                <li>
                ...
                <li>
                  Verify the{" "}
                  <code className="bg-gray-100 px-1 rounded">redirect_uri</code>{" "}
                  matches what's in the metadata
                </li>
                <li>
                  Store the{" "}
                  <code className="bg-gray-100 px-1 rounded">
                ...
                <li>
                  Display a consent screen showing client.dev's information
                </li>
              </ol>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-gray-600 mb-3">
              This is the metadata document your server will fetch during the
              OAuth flow:
            </p>
            <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="flex items-start space-x-3">
              <div className="text-sm">
                ...
                <p className="text-amber-800 mt-1">
                  This test opens a new window to your authorization endpoint
                  with PKCE enabled. Make sure your server is configured to
                  accept requests and can fetch metadata from client.dev. The
                  callback will redirect to client.dev/oauth/callback, which
                  will exchange the authorization code for tokens.
                </p>
              </div>
            </div>
          </CardContent>
